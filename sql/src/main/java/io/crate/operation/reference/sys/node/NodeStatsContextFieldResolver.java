...
public class NodeStatsContextFieldResolver {
    ...
    public NodeStatsContext forColumns(Collection<ReferenceIdent> columns) {
        NodeStatsContext context = NodeStatsContext.newInstance();
        if (columns.isEmpty()) {
            return context;
        }

        List<ColumnIdent> processedColumnIdents = new ArrayList<>();
        for (ReferenceIdent column : columns) {
            ColumnIdent columnIdent = column.columnReferenceIdent().columnIdent();
            if (SysNodesTableInfo.IDENT.equals(column.tableIdent())) {
                if (!processedColumnIdents.contains(columnIdent)) {
                    consumerForColumnIdent(columnIdent).accept(context);
                    processedColumnIdents.add(columnIdent);
                }
            } else {
                throw new IllegalArgumentException(
                    String.format("Cannot resolve NodeStatsContext field for \"%s\" table ident.", column.tableIdent())
                );
            }
        }
        return context;
